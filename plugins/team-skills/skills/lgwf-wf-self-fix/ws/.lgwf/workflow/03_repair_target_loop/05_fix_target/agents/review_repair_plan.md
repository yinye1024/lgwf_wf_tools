...
- `.lgwf/target_run_health.json`: 最近一轮运行的 warning、fallback 和 Codex retry 摘要；通常只作为审查背景。
...
- 如果根因是 `contract_drift` 或 `output_contract`，计划是否明确同步 root `workflow.lgwf` 拓扑与 finalize/verify/prompt/README 等输出契约。
- 是否错误地把 `data_fallback`、Codex retry 或 HTTP fallback 当成必须修改 target source 的根因。
