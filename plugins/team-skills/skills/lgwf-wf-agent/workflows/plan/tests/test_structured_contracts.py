...
class StructuredContractsTest(unittest.TestCase):
    def test_lgwf_plan_uses_isolated_wf_and_ws_layout(self) -> None:
        self.assertTrue((ROOT / "workflow.lgwf").is_file())
    ...
    def test_human_facing_test_and_prompt_text_has_no_mojibake(self) -> None:
        ...
        for relative in checked_files:
            base = PACKAGE_ROOT if relative.startswith("tests/") else ROOT
    ...
    def test_confirmation_context_includes_summary(self) -> None:
        ...
        plan = {
            "summary": {
                "problem_statement": "测试确认摘要",
                "target_type": "modify_artifact",
                "workflow_flow": ["Task 1"],
            },
            "tasks": [
                {
                    "task_id": "task-1",
                    ...
                    "produced_artifacts": ["src/task.py"],
        ...
        self.assertIn("plan_contract_safety", confirmation)
    ...
    def test_resolve_max_attempt_decision_routes_user_choices(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(resolver.normalize_action({"value": {"action": "approve"}}), "continue")
