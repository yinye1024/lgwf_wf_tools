...
- 目标类型：区分 `create_artifact`、`modify_artifact`、`execute_process`、`analyze`、`fix`、`review`。
- 执行主体：区分当前 run 要做的实施/验证动作，和目标产物未来运行时的内部行为。
...
## Target Type And Task Role

计划必须显式声明目标类型和每个 task 的角色，防止把目标产物未来运行时的行为当成当前执行任务。

- 顶层 `summary.target_type` 必须是：`create_artifact`、`modify_artifact`、`execute_process`、`analyze`、`fix`、`review` 之一。
- 每个 task 必须包含 `task_role`：`implementation_action`、`validation_action`、`generated_artifact_behavior`、`human_decision` 之一。
- 每个 task 必须包含 `execution_subject`，说明执行主体，例如 `current_lgwf_plan_run`、`target_artifact_files`、`generated_artifact_runtime`。
- 每个 task 必须包含 `produced_artifacts`，列出当前 task 会创建、修改或验证的目标文件、目录、代码、配置、文档或报告。
- 当 `summary.target_type` 是 `create_artifact` 时，当前执行计划不得包含 `task_role=generated_artifact_behavior` 的 task。
- 当 `summary.target_type` 是 `create_artifact` 时，目标产物未来运行时的业务节点只能写入待生成 artifact 的文件内容、设计文档或 workflow 定义中，不得作为当前 run 的 task 执行。

## Human Approval Task Modeling

计划必须区分“当前 `lgwf-plan` run 真实需要用户拍板”和“目标产物未来运行时包含的 APPROVAL 节点”。

当人工确认属于当前 run 时，必须建模为独立 `human_decision` task，不得把确认后的 artifact 塞进确认前 task 的验收条件。
...
当 `summary.target_type=create_artifact` 且目标产物是 workflow、skill 或插件时，目标产物未来运行时的 APPROVAL 节点不得作为当前 run 的 task。此时：

- `confirm_requirements`、`confirm_business_flow`、`confirm_step_designs` 等未来节点只能写入目标 artifact 的 `workflow.lgwf`、确认模板、脚本、文档和测试契约。
- 当前 run 的 task 应使用 `implementation_action` 或 `validation_action`，例如 `implement_requirements_stage`、`implement_business_flow_stage`、`implement_step_design_stage`、`verify_generated_workflow_package`。
- 当前 run 的 `produced_artifacts` 可以包含目标 artifact 内的确认模板、固化脚本和测试文件，但不得包含当前 work dir 的未来运行时确认记录，例如 `.lgwf/create_requirements_approval.json`、`.lgwf/create_requirements.json`、`.lgwf/business_flow.json`、`.lgwf/step_designs.json`，除非这些文件是作为目标 package 的示例、fixture 或文档明确生成。
- 对 `lgwf-wf-create` 这类目标 workflow，步骤设计阶段在目标 artifact 内应体现 `design_step_documents`、`confirm_step_designs`、`finalize_step_designs` 的未来运行时链路，但当前 `lgwf-plan` task 只能负责生成这套链路对应的文件和验证。
