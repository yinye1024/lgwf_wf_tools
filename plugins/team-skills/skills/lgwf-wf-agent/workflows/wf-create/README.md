...
`lgwf-wf-create` 用于根据用户原始意图创建一个新的 LGWF workflow 初稿。当前目录已经是 `lgwf-wf-agent/workflows/wf-create` 下的内部 workflow package：外层承载说明、测试和固定 `ws/`，真实 workflow package root 位于 `wf/`，由 facade 根目录 `registry.json` 以 `wf-create` 派发。
...
- package root：`plugins/team-skills/skills/lgwf-wf-agent/workflows/wf-create`
...
```powershell
python -m unittest discover plugins\team-skills\skills\lgwf-wf-agent\workflows\wf-create\tests
```
