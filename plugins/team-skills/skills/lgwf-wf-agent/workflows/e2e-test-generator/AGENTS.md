...
本目录是 `lgwf-wf-agent` facade 下的内部 workflow package，职责是为一个已有 LGWF workflow 生成端到端测试骨架和验收入口。它不是独立 Codex skill，不得单独注册；外部只能通过 `lgwf-wf-agent` 根目录 `SKILL.md` 和 `registry.json` 派发到本目录的 `workflow.lgwf`。
...
python $lgwfPy run --workflow-lgwf plugins\team-skills\skills\lgwf-wf-agent\workflows\e2e-test-generator\workflow.lgwf --work-dir <registry-work-dir> --input-json <json> --background
...
```powershell
$lgwfPy = "plugins\team-skills\skills\lgwf-wf-agent\vendor\lgwf-client-assist\scripts\lgwf.py"
python $lgwfPy audit plugins\team-skills\skills\lgwf-wf-agent\workflows\e2e-test-generator\workflow.lgwf
python -m unittest discover plugins\team-skills\skills\lgwf-wf-agent\workflows\e2e-test-generator\tests
```
