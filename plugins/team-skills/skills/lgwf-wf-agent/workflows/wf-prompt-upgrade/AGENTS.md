...
本目录是 `lgwf-wf-agent` facade 下的内部 workflow package，不是独立 Codex skill。它专门处理“目标 workflow 引用的 prompt 是否具备足够强的设计能力、上下游契约和质量控制”，并在用户确认后把升级方案应用到目标 workflow package。
...
1. `init_prompt_upgrade_target`：确认目标 workflow 信息，并写入 `.lgwf/prompt_upgrade_target.json`。
2. `check_lgwf_client_assist`：检查 facade bundled `lgwf-client-assist` 是否可用；缺少时直接失败并报告，不 fallback 到用户 `.codex` 目录的外部 skill。
3. `build_prompt_inventory`：盘点目标 workflow 引用的 prompt，生成 inventory。
4. `design_prompt_upgrade`：使用 ReAct 三段式生成升级设计。
...
5. `prepare_prompt_upgrade_confirmation`：把 proposal、review 和风险整理成人工确认上下文。
6. `confirm_prompt_upgrade`：向用户展示升级项、文件范围、质量指标、验收检查和风险，让用户批准全部、批准部分或拒绝。
7. `validate_prompt_upgrade_decision` 和 `route_after_prompt_upgrade_decision`：校验用户决策并路由。
8. `apply_prompt_upgrade`：仅当用户 approve 时运行，按确认范围生成 apply plan、修改文件并复核。
9. `summarize_prompt_upgrade`：无论 apply 或 reject，最终输出 summary。
