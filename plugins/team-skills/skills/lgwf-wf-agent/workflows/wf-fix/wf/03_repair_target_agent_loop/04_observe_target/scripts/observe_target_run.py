...
def find_pending_human_request(work_dir: Path) -> dict | None:
    human_dir = work_dir / ".lgwf" / "human"
    if not human_dir.is_dir():
        return None
    requests = sorted(human_dir.glob("*.request.json"), key=lambda path: path.stat().st_mtime, reverse=True)
    for request_path in requests:
        request_id = request_path.name.removesuffix(".request.json")
        if (human_dir / f"{request_id}.response.json").exists():
            continue
        try:
...
def latest_run_failed(run_artifacts: dict) -> bool:
    latest = run_artifacts.get("latest_run") if isinstance(run_artifacts, dict) else None
    if not isinstance(latest, dict):
        return False
    return str(latest.get("status") or "").lower() == "failed"
    ...
    *,
    ...
    if has_repairable_contract_issue(issues):
        ...
        observation = {
            ...
            "phase": phase,
            "status": "needs_repair",
            "failure_class": "contract_drift" if phase == "contract_drift" else "output_contract",
        ...
        write_current_artifact(lgwf_root, "observation", observation)
        ...
        return {
            "target": target,
            "target_repair_observation": observation,
...
def main() -> None:
    ...
    current = load_current_artifact(root, "run", {})
    ...
    pending_request = find_pending_human_request(work_dir) if not request_id else None
    if pending_request:
    ...
    if phase == "waiting_human" or request_id:
        ...
        if pending_request:
            approval["request"] = pending_request
        if request_id:
        ...
        write_current_artifact(root, "approval", approval)
        write_current_artifact(root, "observation", approval)
    ...
    if not completed:
        ...
        observation = {
            ...
            "phase": phase,
            "status_detail": status,
            "wait_stdout": wait_proc.stdout,
        }
        write_current_artifact(root, "observation", observation)
    ...
    failed = phase == "failed" or bool(status.get("last_error")) or status_proc.returncode != 0 or latest_run_failed(summary)
    if failed:
        observation = {
            "attempt": target.get("current_attempt"),
            "phase": phase,
        ...
        write_json(root / "self_fix_target.json", target)
        output_state({"target": target, "target_repair_observation": observation, "next_action": "fix"})
        return
