...
本目录是 `lgwf-wf-agent` facade 下的内部 workflow package，职责是运行一个目标 LGWF workflow，观察失败或阻塞原因，并在候选目录中自动修复目标 workflow package，验证通过后再 promote 回真实目标目录。它不是独立 Codex skill，不得单独注册；外部只能通过 `lgwf-wf-agent` 根目录 `SKILL.md` 和 `registry.json` 派发到本目录的 `wf/workflow.lgwf`。
...
python $lgwfPy run --workflow-lgwf plugins\team-skills\skills\lgwf-wf-agent\workflows\wf-fix\wf\workflow.lgwf --work-dir plugins\team-skills\skills\lgwf-wf-agent\workflows\wf-fix\ws --input-json "{}" --background
...
```powershell
$lgwfPy = "plugins\team-skills\skills\lgwf-wf-agent\vendor\lgwf-client-assist\scripts\lgwf.py"
python $lgwfPy audit plugins\team-skills\skills\lgwf-wf-agent\workflows\wf-fix\wf\workflow.lgwf
python -m unittest discover plugins\team-skills\skills\lgwf-wf-agent\workflows\wf-fix\tests
```
