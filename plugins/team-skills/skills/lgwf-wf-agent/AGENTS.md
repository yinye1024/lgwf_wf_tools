...
2. 读取目标 `AGENTS.md`，确认适用场景、输入契约、approval 语义、固定输出和自检要求。
3. 根据目标 workflow 的输入契约准备 `--input-json`。涉及中文或复杂 JSON 时，优先写入 UTF-8 文件，再由脚本读取或用安全 argv 传递。
4. 使用 bundled client 启动或继续：
...
5. 保存同一个 `session_id` / `pid` / `work_dir`，使用 `status`、`wait`、`approval` 和 `runs` 持续推进；不要把后台启动当一次性命令。
6. 进入 `waiting_human` 时展示 workflow 给出的摘要、选项和风险，只提交用户明确确认的结果。
7. 结束后汇总最终状态、关键产物、变更文件、阻塞项和建议下一步。
