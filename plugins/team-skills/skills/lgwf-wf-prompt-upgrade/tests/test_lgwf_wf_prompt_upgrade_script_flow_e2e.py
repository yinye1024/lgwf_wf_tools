...
TARGET_ROOT = Path(r"D:\allen\github\lgwf_plugins\plugins\team-skills\skills\lgwf-wf-prompt-upgrade")
...
def patch_skill_discovery_env(workspace: Path, *, skill_dir: Path | None):
    env = {
        ...
        "HOMEDRIVE": "",
        "HOMEPATH": "",
        "CODEX_HOME": str(workspace / "fake-codex-home"),
    }
...
class LgwfWfPromptUpgradeScriptFlowE2ETest(unittest.TestCase):
    ...
    def test_case_environment_check_success_writes_artifact(self) -> None:
        with isolated_workspace_cwd() as workspace:
            ...
            (skill_dir / "SKILL.md").write_text("# fake lgwf-client-assist\n", encoding="utf-8")
    ...
    def test_case_environment_check_missing_skill_fails_fast(self) -> None:
        with isolated_workspace_cwd() as workspace:
            with patch_skill_discovery_env(workspace, skill_dir=None):
                payload, _, error = run_script_main(
                    "00_check_environment/scripts/check_lgwf_client_assist.py",
                    "case_environment_check_missing_skill_fails_fast",
                )
            ...
            self.assertIn("lgwf-client-assist", artifact["reason"])
            self.assertTrue(artifact["checked"])
            self.assertEqual(str(error), artifact["reason"])
            self.assertFalse((workspace / ".lgwf" / "prompt_upgrade" / "inventory.json").exists())
