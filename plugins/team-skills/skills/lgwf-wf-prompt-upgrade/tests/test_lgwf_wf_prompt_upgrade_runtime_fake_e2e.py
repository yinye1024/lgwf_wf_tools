...
def find_lgwf() -> Path:
    ...
    return codex_home / "skills" / "lgwf-client-assist" / "scripts" / "lgwf.py"
...
def write_fake_skill_dir(root: Path) -> Path:
    skill_dir = root / "fake-codex-home" / "skills" / "lgwf-client-assist"
    skill_dir.mkdir(parents=True, exist_ok=True)
    (skill_dir / "SKILL.md").write_text("# lgwf-client-assist\n", encoding="utf-8")
    return skill_dir
...
class PromptUpgradeRuntimeFakeEndToEndTest(unittest.TestCase):
    ...
    def _run_scenario(self, scenario_id: str, confirm_value: dict[str, Any], confirm_decision: str) -> tuple[Path, dict[str, Any]]:
