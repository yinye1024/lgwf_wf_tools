...
3. 运行内置 workflow，并把仓库路径和摘要范围作为 `input-json` 传入。
...
```powershell
python plugins/team-skills/skills/lgwf-wf-tools/vendor/lgwf-client-assist/scripts/lgwf.py run plugins/team-skills/skills/git-diff-brief/wf/workflow.lgwf --work-dir plugins/team-skills/skills/git-diff-brief/ws --input-json "{\"repo_path\":\".\",\"summary_scope\":\"worktree git diff + latest commit\"}"
```

如果 `ws/.lgwf` 已存在，先检查当前 session 状态，再决定 resume、rerun 或保留现场。
