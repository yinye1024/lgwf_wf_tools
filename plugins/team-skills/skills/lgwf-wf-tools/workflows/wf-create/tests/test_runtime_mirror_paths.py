...
class RuntimeMirrorPathsTest(unittest.TestCase):
    ...
    def test_scaffold_package_loads_resources_and_validator_from_runtime_workflow_mirror(self) -> None:
        ...
        self.assertEqual(plan["target_package_root"], "plugins/team-skills/skills/git-diff-brief")
    ...
    def test_prepare_implementation_context_resolves_target_from_repo_root_not_run_cwd(self) -> None:
        ...
        self.assertEqual(context["target_package_root"], "plugins/team-skills/skills/git-diff-brief")
        self.assertEqual(Path(context["workspace_root"]), self.work_dir)
        self.assertEqual(
            Path(context["target_package_abs"]),
            self.work_dir / "plugins" / "team-skills" / "skills" / "git-diff-brief",
        )
    ...
    def test_summarize_create_result_uses_current_target_package_from_stdin(self) -> None:
        ...
        self.assertEqual(result["target_package_root"], "plugins/team-skills/skills/git-diff-brief")
    ...
    def test_summarize_create_result_falls_back_to_implementation_result_when_stdin_empty(self) -> None:
        write_json(
            self.work_dir / ".lgwf" / "implementation_result.json",
            {
                "workflow_name": "git-diff-brief",
                "target_package_root": "plugins/team-skills/skills/git-diff-brief",
                "generated": {"root_files": ["plugins/team-skills/skills/git-diff-brief/AGENTS.md"]},
                "verification": [
                    {
                        "command": "python -m unittest discover plugins\\team-skills\\skills\\git-diff-brief\\tests",
                        "result": "passed",
                    }
                ],
            },
