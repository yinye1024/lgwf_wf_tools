...
4. 按 `dsl-assist` 的 workflow 创建规范生成根 workflow 和子 workflow：根 workflow 只编排业务阶段，阶段内节点放在对应子 workflow。
5. 记录本轮实际生成的文件、目录、占位内容和剩余风险。
6. 若某些步骤只能生成占位内容，明确说明原因和后续补齐点。
...
- 生成的 workflow 拓扑遵循 `dsl-assist`：根 workflow 保持薄编排，子 workflow 路径可被递归审计，REACT 的 `ACT WORKFLOW` 只引用包内相对路径。
