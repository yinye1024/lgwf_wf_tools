...
## Output
将修订阶段的 approval decision record 写入 `.lgwf/create_requirements_revision_approval.json`，供后续 route 和正式固化读取。
...
## Constraints
- 只输出 `.lgwf/create_requirements_revision_approval.json` 对应的 approval decision record。
