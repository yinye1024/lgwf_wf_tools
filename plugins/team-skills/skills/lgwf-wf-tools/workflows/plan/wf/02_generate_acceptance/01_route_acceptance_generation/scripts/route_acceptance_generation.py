...
def format_confirmation(plan: dict, acceptance: dict, request: dict | None = None) -> dict:
    ...
    return {
        ...
        "plan_contract_safety": load_safety_validator().validate_plan_contract(request or {}, plan),
        "tasks": rows,
...
def main() -> None:
    root = Path.cwd()
    request = load(root / ".lgwf" / "react_task_request.json")
    ...
    confirmation = format_confirmation(plan, acceptance, request) if ok else direction
