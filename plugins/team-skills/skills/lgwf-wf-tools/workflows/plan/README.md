...
从 facade 根目录 `plugins\team-skills\skills\lgwf-wf-tools` 运行：
...
```powershell
python -m unittest discover plugins\team-skills\skills\lgwf-wf-tools\workflows\plan\tests
```

本次计划安全契约的最小验证命令：

```powershell
python -m pytest plugins/team-skills/skills/lgwf-wf-tools/workflows/plan/tests/test_lgwf_plan_script_flow.py
python -m pytest plugins/team-skills/skills/lgwf-wf-tools/workflows/plan/tests/test_structured_contracts.py -k "plan_generation_spec or confirmation_template or confirmation_context"
```
...
python plugins\team-skills\skills\lgwf-wf-tools\workflows\plan\tests\test_lgwf_plan_real_positive_e2e.py
```
