...
`eval` 和 `trace-eval` 不评价同一件事：`eval` 检查 facade 规则、baseline 和本地 override 风险；`trace-eval` 消费 LGWF runtime 的 `trace.json` 和 `eval-suite.json`，检查真实运行轨迹、client call、catalog policy 和 unexpected route。`pre-release` 默认会在 `workflow-health` 之后运行 `trace-eval`，再生成 scorecard 和 upgrade report。
