...
4. 主 agent handoff：把已确认需求、业务流和已落盘 scaffold 交给主 agent；主 agent 先生成执行计划，再按计划完善并验证目标 package。

主 agent 后续实现必须先把上下文检查、目标实现和验证写入执行计划，再显式遵守本文的目录、状态和验证边界逐项实施；如果已落盘 scaffold 无法支撑需求，应先更新计划，并在最终说明中列出保守假设和调整理由，不得静默改变模块边界。
