...
当前主干会先确认目标范围，然后用 `FOREACH` 逐个处理 `state.wf_dsl_upgrade.targets`。`mode=dry_run` 只 audit 和汇总，不写目标文件；`mode=apply` 需要范围确认 `approve` 后，才允许 per-target Codex 修复当前 `.lgwf` 文件。
