...
`wf-create-fast` 是 registry 中唯一对外可见、可启动的创建 workflow 入口。它确认需求、确认业务流、落盘 scaffold，并通过 HANDOFF 交给主 agent 继续完善。`wf-create` 不在 registry 中，不作为可选择或可启动入口。
...
契约：`workflows/wf-audit-fix/entry_contract.json`。
...
`target_paths` 可传 `.lgwf` 文件或目录；目录会递归扫描其中所有 `.lgwf` 文件并生成目标 manifest。

当前主干会先确认目标范围，然后用 `FOREACH` 逐个处理 `state.wf_audit_fix.targets`。`mode=dry_run` 只 audit 和汇总，不写目标文件；`mode=apply` 需要范围确认 `approve` 后，才允许 per-target Codex 修复当前 `.lgwf` 文件。
...
`objective`、`request`、`target_type` 必填；`analysis_target_files` 和 `analysis_target_dirs` 至少提供一个。

## tool-workflow

`target-run`、`self-improve` 和 `self-improve-seed` 使用 CLI 参数，不走 LGWF `--input-json`。对应入口契约分别是：
