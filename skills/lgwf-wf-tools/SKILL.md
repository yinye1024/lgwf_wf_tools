---
name: lgwf-wf-tools
description: 当用户调用 /lgwf-wf-tools，或需要 LGWF 入门引导、工作流入口路由、运行监控、审批处理、初始化、诊断、列表或自我优化时使用。
---
...
- 用户希望快速了解 LGWF、学习如何提问或了解 `lgwf-wf-tools` 的使用入口。
...
- `approve` 是纯决策，不得携带 `--value-json`；节点应自行固化已展示的 proposal。只有 `review revise` 才提交完整业务 JSON value。
- 提交 `review revise` value 时，包含中文或复杂嵌套的 JSON 优先使用 `scripts/safe_approval_submit.py --value-file` 或 `--value-json-base64`；只有纯 ASCII 小 payload 才直接使用 `review submit --value-json`。
...
| `/lgwf-wf-tools guide` | 读取 [AGENTS.md](AGENTS.md)，路由到 `lgwf-guide`。 |
| `/lgwf-wf-tools learn` | 读取 [AGENTS.md](AGENTS.md)，路由到 `lgwf-guide`。 |
| `/lgwf-wf-tools 入门` | 读取 [AGENTS.md](AGENTS.md)，路由到 `lgwf-guide`。 |
