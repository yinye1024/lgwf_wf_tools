...
def _required_input_missing(contract: dict[str, Any], input_json: str | None, input_json_file: str | None) -> bool:
    return contract.get("input_mode") == "input_json_required" and input_json is None and input_json_file is None
...
def _prepare_contract_args(args: list[str], temp_dir: Path) -> list[str]:
    prepared = list(args)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="lgwf-wf-tools-input-") as tmp:
            prepared_args = _prepare_contract_args(args, Path(tmp))
