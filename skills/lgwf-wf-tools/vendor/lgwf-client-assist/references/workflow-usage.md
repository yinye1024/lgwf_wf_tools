...
- 输入 `rerun`：清理 `<work_dir>` 下 LGWF runtime 自己的旧运行数据，以及 workflow 在 `artifact_contracts.json` 中明确声明的 `run_managed_workspace_outputs`，然后重新运行 workflow。普通 workspace 文件默认保留。
...
rerun 默认只清理 runtime 固定路径，例如 `.lgwf/input_state.json`、`.lgwf/context.json`、`.lgwf/runs`、`.lgwf/checkpoints`、`.lgwf/processes`、`.lgwf/human`、`.lgwf/codex`、`.lgwf/main_agent`、`.lgwf/logs` 和 `.lgwf/workflow` snapshot。workflow 若需要在 rerun 前清理 workspace 中的可重建中间产物，必须在 `artifact_contracts.json` 中声明：

```json
{
  "run_managed_workspace_outputs": [
    "docs/steps",
    ".lgwf/wf_create"
  ]
}
...
`run_managed_workspace_outputs` 只能包含相对 `<work_dir>` 的安全路径；禁止绝对路径、盘符路径、`..`、指向 `<work_dir>` 本身、整个 `.lgwf` 目录，以及 symlink 或 junction 路径。未声明的 workspace 文件不会因为 rerun 被删除。
