...
- `AGENT_LOOP` 默认 `TOKEN_MAX 1000000`，默认 Codex analysis 授权读取 `state.targets.dirs` 和 `state.targets.files`，并用 `state.targets.edit_dirs` / `state.targets.edit_files` 表达可修改目录和文件；slot 内不得覆盖 `ANALYSIS_DIRS` / `ANALYSIS_FILES` / `EDIT_DIRS` / `EDIT_FILES`。
