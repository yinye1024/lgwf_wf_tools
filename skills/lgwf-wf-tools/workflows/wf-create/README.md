...
- `03_confirm_step_designs` 的步骤设计 ReAct slot workflow、prompt、质量反馈和 gate 后自动固化契约。
...
- `scaffold_package`：定义目标目录、关键文件、占位物、相对路径规则和运行状态边界；脚手架阶段只生成 `.lgwf/scaffold_package_result.json` 中的 `scaffold_plan/create_files`，不创建或覆盖目标 package 真实文件，也不向目标 package 根目录写入 `.lgwf`。如果已确认需求明确目标是 Codex skill、包含根 `SKILL.md`，或列出 `scripts/`、`tests/`、`wf/shared/scripts/` 下的目标文件，脚手架会推断 `skill_wrapped_workflow` profile，并把这些语义必需文件写入 `create_files`。

步骤设计和实现阶段当前已补齐以下契约：

- `prepare_dsl_reference_context`：从 facade 内置 bundled client 复制 `dsl-assist` 规范到 `.lgwf/create_reference_context/dsl-assist/`；从 facade docs 复制 workflow 模块化创建指引到 `.lgwf/create_reference_context/workflow-modular-development/`，复制 Contract 摘要到 `.lgwf/create_reference_context/module-contract/`，并发布 `.lgwf/create_reference_context/step-design-reference-index.md` 和 `.lgwf/create_reference_context/implementation-reference-index.md`。其中 step design index 和 `.lgwf/create_reference_context/` 会直接注入给首轮步骤设计 Codex；implementation index 作为生成期参考产物保留，初版实现 Codex 不再读取该索引或整个 `.lgwf/create_reference_context` 目录。
- `step_design_proposal`：先由 `build_step_design_contract` 同时生成动态校验契约和 bounded authoring context，聚合已确认需求、已确认业务流、scaffold plan、schema 约束和 required files/stages，再由 `CODEX generate_step_designs` 读取 `.lgwf/create_reference_context/step-design-reference-index.md` 和 `.lgwf/create_reference_context/` 中的 DSL/module 参考资料，首轮生成完整结构化 `.lgwf/step_designs_proposal.json`。proposal 必须覆盖 step、directory 和 file 三级设计；workflow/prompt 文件必须提供 `exact_content`，脚本、Markdown、JSON contract 和测试文件必须提供具体结构化 contract。随后 Python normalize/初检并进入修复 ReAct；normalize 只做机械归一化，不补齐缺失设计，required file、stage workflow、target 引用和 coverage 缺口必须由 structural gate 暴露给 repair。`REASON CODEX` 读取 observation/decision 生成 `.lgwf/step_design_repair_plan.json`，`ACT CODEX` 执行修复，`OBSERVE PY` 验收，`DECIDE PY` 决定继续或退出。
...
- `apply_validated_step_designs`：在 structural gate 通过且 `proposal_hash` 匹配当前 proposal 后固化 `.lgwf/step_designs.json`。
...
- 脚手架只生成目标 package 框架计划，不创建或覆盖目标 package 真实文件，不向目标 package 根目录写入 `.lgwf`；运行状态边界仍归 `ws/.lgwf`。
...
- `proposal`：`step_design_proposal` 产出的完整结构化 `.lgwf/step_designs_proposal.json`，用于 structural gate 校验和后续固化；该 proposal 包含 `directory_designs[]`、`file_designs[]` 和 `step_designs[]`，只描述文件结构和契约，不承载最终源码。
- `gate observation`：`validate_step_designs_structure.py` 产出的 `.lgwf/step_design_observation.json`，记录 structural gate 是否通过和当前 `proposal_hash`。
- `confirmed artifact`：structural gate 通过且 hash 匹配后自动固化的 `.lgwf/step_designs.json`。

步骤设计不再进入人工 `approve` / `revise` / `reject` REVIEW；如果 structural gate 仍失败，proposal 修复留在 `02_step_design_proposal` 的 repair ReAct 内闭环。只有 `.lgwf/step_design_observation.json.passed=true` 且 `proposal_hash` 匹配当前 proposal 时，`03_step_design_review` 才会自动固化 `.lgwf/step_designs.json`。

步骤设计在 `build_step_design_contract` 准备动态校验契约和 bounded 输入后，由首轮 `CODEX generate_step_designs` 写出 `.lgwf/step_designs_proposal.json`，随后立即执行 Python structural gate，写出 `.lgwf/step_design_structural_gate.json` 和 `.lgwf/step_design_observation.json`。失败结果以结构化 observation 和 decision 反馈给下一轮 `REASON CODEX`，由其生成详细修复方案 `.lgwf/step_design_repair_plan.json`；auto-human 也不能绕过该节点。normalize 不得替 repair 生成缺失的业务设计条目。ReAct 最多 2 轮后仍失败时由 `assert_step_designs_proposal_quality_gate` 基于 `.lgwf/step_design_observation.json` 终止。
...
- `02_repair_implementation_react/agents/spec.md` 是 repair ReAct 的唯一全局 spec。repair 只能基于 `.lgwf/implementation_audit_result.json`、`.lgwf/implementation_observe.json` 和已确认设计契约生成最小修复计划，`act_repair` 消费 `.lgwf/implementation_repair_reason.json` 与 `.lgwf/implementation_context.json`，用 `target_package_abs` 定位目标包，并只能修改其中 `target_files` 指定的文件。
...
- `observe_repair` 先由 `prepare_lgwf_dsl_audit_target.py` 准备固定 TOOL 输入，再用原生 `TOOL run_lgwf_dsl_audit USE lgwf_dsl_cli` 执行根 workflow DSL audit，最后由 `audit_current_implementation.py` 聚合 TOOL 结果和非 DSL 检查，并写入 `.lgwf/implementation_audit_result.json` / `.lgwf/implementation_observe.json`，反馈给下一轮 `reason_repair`。audit 结果保留根 `audit` 字段，同时通过 `workflow_audits` 收集每个 `wf/**/workflow.lgwf` 的 authoring audit 明细，避免递归 audit 在第一个子 workflow 语法错误处停止后只能一轮修一个文件。
...
- `wf/artifact_contracts.json` 声明 `prepare_dsl_reference_context` 复制出的 `dsl-assist` workspace context 文件、`.lgwf/create_reference_context/step-design-reference-index.md`、`.lgwf/create_reference_context/implementation-reference-index.md` 索引文件，以及步骤设计首轮使用的 `.lgwf/step_design_authoring_context.json` 和 `.lgwf/create_reference_context/`。
- 需求和业务流两个 approval 节点使用 `ROUTE_ON_DECISION`、`PERSIST` 和 `approve/revise/reject` 业务路由；步骤设计不再使用人工 approval。
- proposal/实现阶段的 Codex 节点声明 `OUTPUT_JSON`，并与 prompt 输出契约一致。
- 需求 proposal 的 quality gate 在对应 REVIEW 前执行；步骤设计 proposal 的 structural gate 在自动固化前执行；业务流 proposal 由单个 Codex 节点产出，并交给后续人工 REVIEW 确认。
- 需求和业务流确认后固化脚本会生成 `.lgwf/create_requirements.json`、`.lgwf/business_flow.json`；步骤设计由 `apply_validated_step_designs` 生成 `.lgwf/step_designs.json`。
- `prepare_dsl_reference_context` 会复制 `dsl-assist` 的 `guide.md`、`create-workflow.md` 和 `workflow-audit-checklist.md`，并发布 step design 与 implementation 两个 reference index；当前 03 的首轮设计会直接注入 `.lgwf/create_reference_context/step-design-reference-index.md` 和 `.lgwf/create_reference_context/`，同时读取 `.lgwf/step_design_authoring_context.json`、动态 contract、schema 和 passing example。初版实现 Codex 只依赖当前 unit context 和单 unit workflow 的静态 DSL 写作约束。
...
- `implement_steps_react` 通过 `04_implement_steps_react/workflow.lgwf` 先运行 `01_implement_units` 初版 FOREACH，再运行 `02_repair_implementation_react` 修复 ReAct；`implementation_audit_result.json` 会传给 summary，observe/decision 是 repair 内部循环产物，不传给 summary 或 handoff；它仍不把 prompt 修复、agent 化和跨 workflow 自动修复纳入当前范围。
