...
- 创建或修改 `workflow.lgwf` 时必须以当前 implementation unit context 和静态 DSL 写作约束为输入，并通过 authoring audit/递归 audit 验收；根 workflow 保持薄编排，阶段细节优先拆到自包含子 workflow 或复杂 step，所有引用路径保持包内相对路径。
