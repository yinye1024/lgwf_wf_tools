...
- 已确认需求中显式列出的 package 源文件，例如 `scripts/*.py`、`tests/*.py`、`wf/shared/scripts/*.py`，必须进入 `create_files`；这些文件后续会作为步骤设计 structural gate 的 required file design。
- `create_dirs` 与 `create_files` 只描述目标 package 框架，不描述运行状态文件。
- `scaffold_package` 不创建或覆盖目标 package 实体文件；真实文件只由 implementation 阶段从 staging 发布。
