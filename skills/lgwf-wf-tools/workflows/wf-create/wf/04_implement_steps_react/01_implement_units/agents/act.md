...
## Role
你是步骤实现 ACT 子流程的总说明。本阶段已拆为 `ACT WORKFLOW implement_units`：先由脚本准备 implementation units，再通过 `FOREACH` 调用 `01_implement_one_unit/workflow.lgwf`，最后由脚本合并为 `.lgwf/implementation_result.json`。
...
- `.lgwf/scaffold_package_result.json`：确定性脚手架结果，`scaffold_plan` 是生成文件清单、目录结构、placeholder 和 profile 语义的参考。
- `.lgwf/create_reference_context/implementation-reference-index.md`：实现阶段 DSL、audit 和模块化参考路由；需要创建或修复 `workflow.lgwf` 时先读索引，再按需读取 `.lgwf/create_reference_context` 下的具体资料。
...
2. `FOREACH implement_each_unit` 对每个 unit 启动 `01_implement_one_unit/workflow.lgwf`，由 `01_implement_one_unit/agents/act_unit.md` 约束单个 Codex 的实际写入范围。
3. 如果 `.lgwf/implementation_observe.json` 存在且 audit 失败，本轮只生成与失败项相关的 units；无法归因时才回退到根编排和阶段 unit。
4. `merge_implementation_results.py` 合并每个 unit 的结果，记录本轮实际生成的文件、目录、占位内容、剩余风险和本轮处理的 audit 反馈。

## Resume And Budget Rules
- 如果目标目录里已存在的目标 package 只有部分文件，把它视为本轮可续写草稿；先读取精简文件清单，先补齐缺失的必需文件，除非 audit 反馈要求，不重写已经成型的文件。
- resume 后优先完成各阶段 `workflow.lgwf`、阶段私有 `agents/`、`scripts/`、`resources/`、根契约和最小测试这些必需项，再考虑说明性优化。
