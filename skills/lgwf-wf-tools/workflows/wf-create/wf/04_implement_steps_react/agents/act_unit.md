...
- `state.lgwf_wf_create.current_implementation_unit_target_files`：当前节点允许生成或修改的目标文件清单。
- `state.lgwf_wf_create.current_implementation_unit_target_dirs`：当前 unit 的最小目录边界，用于定位和读取局部上下文；它不授权修改目标文件清单之外的文件。
...
5. 只能生成或修改 `current_implementation_unit_target_files` 中列出的文件；即使某个文件位于 `current_implementation_unit_target_dirs` 内，只要不在目标文件清单内也不得修改。
