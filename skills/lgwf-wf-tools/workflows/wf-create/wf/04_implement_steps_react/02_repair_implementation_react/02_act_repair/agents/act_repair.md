...
- `.lgwf/implementation_repair_reason.json`：唯一修复上下文，由 REASON 根据 Python audit/observe 反馈编译完成。
...
3. 如果需要修复，只读取 `target_package_root` 下 `target_files` 指定的目标文件。
4. 只能写 `.lgwf/implementation_repair_stage/<target_file>` 对应的 staging 文件。
5. 根据 `repair_units[].implementation_guidance` 和 `success_checks` 完成 Python audit 失败项要求的最小修复。
