...
class WorkflowStructureTest(unittest.TestCase):
    ...
    def test_per_target_workflow_contains_repair_loop(self) -> None:
        ...
        self.assertIn("READ state.wf_audit_fix.repair_observation", text)
        self.assertIn("READ state.wf_audit_fix.repair_decision", text)
        ...
        reason_text = (WF_ROOT / "03_upgrade_one_target" / "agents" / "reason.md").read_text(encoding="utf-8")
        spec_text = (WF_ROOT / "03_upgrade_one_target" / "agents" / "repair_spec.md").read_text(encoding="utf-8")
        self.assertIn("repair_observation", reason_text)
        self.assertIn("repair_decision", reason_text)
        self.assertIn("no_progress", reason_text)
        self.assertIn("diagnostic_delta", reason_text)
        self.assertIn("repair_observation", spec_text)
        self.assertIn("repair_decision", spec_text)
