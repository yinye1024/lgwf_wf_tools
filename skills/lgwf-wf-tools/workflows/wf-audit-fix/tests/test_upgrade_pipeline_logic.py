...
class UpgradePipelineLogicTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.audit_current_module = load_module("03_upgrade_one_target/scripts/audit_current_target.py", "audit_current_target")
    ...
    def test_collect_targets_exposes_foreach_state_targets(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            foreach_targets = result["state_updates"]["wf_audit_fix.targets"]
    ...
    def test_audit_current_target_initializes_repair_feedback(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            allowed = root / "allowed"
            allowed.mkdir()
            ...
            original_run_lgwf_audit = self.audit_current_module.run_lgwf_audit
            try:
                self.audit_current_module.run_lgwf_audit = lambda path: {
                ...
                result = self.audit_current_module.audit_target(
                    root,
                    {
                        "target_id": "target_0001",
                        "path": str(target),
                ...
                self.audit_current_module.run_lgwf_audit = original_run_lgwf_audit

            self.assertEqual(result["route"], "repair")
            self.assertEqual(result["observation"]["phase"], "initial_audit")
            self.assertFalse(result["observation"]["changed"])
            self.assertEqual(result["observation"]["diagnostic_count"], 1)
            self.assertEqual(result["repair_decision"]["status"], "initial_failure")
            self.assertEqual(len(result["repair_decision"]["diagnostic_identities"]), 1)
