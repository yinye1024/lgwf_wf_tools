...
## goal
设计 `wf/01_collect_targets/workflow.lgwf`，把入口 `audit_fix_target` 解析、目标路径归一化、`allowed_dirs` 越权保护、`scope_mode` 分流和 `max_targets` 限制固化成唯一授权闸门，生成后续全部阶段都必须复用的 `target_manifest.json` 与 `target_scope_validation.json`。
