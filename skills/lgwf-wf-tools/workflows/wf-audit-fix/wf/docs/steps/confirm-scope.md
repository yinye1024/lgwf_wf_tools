...
- `state.wf_audit_fix.targets`
- `ws/.lgwf/target_manifest.json`
- `ws/.lgwf/target_scope_validation.json`

## 输出

- `state.wf_audit_fix.scope_confirmation_context`
- `state.wf_audit_fix.scope_approval`
- `state.wf_audit_fix.scope_route`
...
- 路由脚本只读取 `RESULT state.wf_audit_fix.confirm_scope_result` 中的 decision；业务 context 不作为控制分支的 decision 来源。
