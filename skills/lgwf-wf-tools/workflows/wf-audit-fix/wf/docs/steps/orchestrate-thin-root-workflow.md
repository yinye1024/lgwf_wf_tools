...
- 依赖文件或状态：
  - `docs_tmp/wf-audit-fix-development.md`
...
- foreach 路径：`FOREACH upgrade_each` 对 `state.wf_audit_fix.targets` 逐项运行 `03_upgrade_one_target/workflow.lgwf`，使用 `FAIL collect` 汇总每个 item 结果。
...
- 在 `wf/artifact_contracts.json` 中声明 `ws/.lgwf/*.json` 关键产物和 `ws/reports/wf-audit-fix/report.md`，避免后续阶段依赖隐式临时文件。
