...
def build_result_summary(root: Path, provided_context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    summary = {
        ...
        "report_path": "reports/wf-audit-fix/report.md",
    }
    return summary
...
def main() -> None:
    ...
    report_path = root / "reports" / "wf-audit-fix" / "report.md"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.write_text(report_text + "\n", encoding="utf-8")
    print(json.dumps({"wf_audit_fix.result_summary": summary}, ensure_ascii=False, indent=2))
