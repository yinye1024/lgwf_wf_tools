...
3. `confirm_upgrade`：把 proposal、review 和风险整理成人工确认上下文，向用户展示升级项、文件范围、质量指标、验收检查和风险，让用户批准全部、通过 `revise` 批准部分或拒绝，并校验用户决策。`approve` 是纯控制面决策，子流程会根据已展示 proposal 生成最终 `.lgwf/prompt_upgrade/decision.json`；拒绝分支在本子流程内部通过 `FAIL_ALL` 终止整个 run，不把 route 决策暴露给根 workflow。
...
- 三种决策含义：
  - 批准全部：提交 `approve` 纯决策，不携带业务 value；workflow 会根据 proposal 生成 `{"approve": true, "approved_upgrade_ids": ["<all>"], "reject": false}`。
  - 批准部分：提交 `revise`，value 必须是完整业务 JSON，例如 `{"approve": true, "approved_upgrade_ids": ["..."], "reject": false}`。
  - 拒绝：提交 `reject` 纯决策；workflow 会生成 `{"approve": false, "approved_upgrade_ids": [], "reject": true}` 并通过 `FAIL_ALL` 终止整个 run。
...
4. 使用同一个 `session_id` / `pid` / `work_dir` 通过 `status`、`wait`、`review`、`approval` 和 `runs` 持续推进。
5. 遇到 `waiting_human` 时按 `workflows/01-share/approval.md` 的人工确认展示模板展示 workflow 给出的 confirmation context。`confirm_prompt_upgrade` 使用 `REVIEW`：`approve` / `reject` 只提交决策；只有部分批准才用 `revise` 提交完整业务 JSON。
