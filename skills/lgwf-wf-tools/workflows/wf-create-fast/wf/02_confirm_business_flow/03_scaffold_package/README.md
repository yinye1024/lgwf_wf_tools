...
根据 `.lgwf/create_requirements.json` 和 `.lgwf/business_flow.json` 生成确定性 `scaffold_plan`，为后续 `03_materialize_scaffold` 和主 agent authoring 提供目标 package 的目录、文件和阶段 manifest。
...
- `state.lgwf_wf_create_fast.scaffold_package_result`
