...
def build_runtime_authoring_package(root: Path) -> Path:
    package_root = root / "wf_convert_authoring_package"
    if package_root.exists():
        shutil.rmtree(package_root)
    shutil.copytree(PACKAGE_ROOT / "wf", package_root)
    child_target = package_root / "workflows" / "wf-create" / "wf"
    child_target.parent.mkdir(parents=True, exist_ok=True)
    shutil.copytree(WF_CREATE_WORKFLOW_ROOT, child_target)
    return package_root / "workflow.lgwf"
                ...
                class _FakeRunWorkflowCapability:
                    ...
                    def create_node(self, node_id, config):
                        def node(state):
                            ...
                            return _write_state_path(dict(state), result_path, result)
