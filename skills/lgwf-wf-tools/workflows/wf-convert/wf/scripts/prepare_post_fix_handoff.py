...
def build_handoff_payload(summary: dict[str, Any], work_dir: Path) -> dict[str, Any]:
    created_workflow = summary.get("created_workflow")
    if not isinstance(created_workflow, dict):
        raise ValueError("wf_create_result_summary 缺少 created_workflow")
    package_root = str(created_workflow.get("target_package_root", "")).strip()
    if not package_root:
        raise ValueError("created_workflow 缺少 target_package_root")
...
def main() -> None:
    ...
    print(json.dumps({"lgwf_wf_convert.post_fix_handoff_payload": payload}, ensure_ascii=False, indent=2))
