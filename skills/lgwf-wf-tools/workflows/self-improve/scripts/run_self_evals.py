...
SELF_EVAL_TRIGGER_PATTERNS = [
    ...
    "workflows/self-improve/**/*.py",
    "workflows/self-improve/**/*.json",
...
def check_registry_paths(registry: dict[str, Any]) -> list[str]:
    ...
    for item in workflows:
        ...
        if kind not in {"lgwf", "tool-workflow"}:
            issues.append(f"{workflow_id}: unsupported kind: {kind}")
            continue
        required = ("workflow_lgwf", "work_dir", "agents_md") if kind == "lgwf" else ("entry", "agents_md")
        for key in required:
            value = item.get(key)
...
def write_markdown(path: Path, report: dict[str, Any]) -> None:
    ...
    lines = [
        f"# lgwf-wf-tools Self Eval {status}",
        "",
