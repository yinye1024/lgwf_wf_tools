...
- 用户主动要求复盘、沉淀 case、生成 proposal 或运行 self eval 时，由 `lgwf-wf-tools` skill 触发。
- 修改 facade 或内部 workflow 指引后，由本地脚本或 CI 运行 `workflows/self-improve/scripts/run_self_evals.py`。
...
python workflows\self-improve\scripts\self_improve.py pre-release --version <version> --source release
python workflows\self-improve\scripts\self_improve.py pre-release --version <version> --source release --run-workflow-tests
python workflows\self-improve\scripts\record_incident.py --type <type> --summary "..." --evidence-json "[...]"
python workflows\self-improve\scripts\create_eval_case.py --incident <incident.json>
