...
def unwrap_dsl_upgrade_target(raw_request: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(raw_request, dict):
        raise TypeError("dsl_upgrade_target 输入必须是 JSON object")
    wrapped = raw_request.get("dsl_upgrade_target")
...
def collect_targets_from_request(root: Path, raw_request: dict[str, Any]) -> dict[str, Any]:
    ...
    request = _normalize_request(unwrap_dsl_upgrade_target(raw_request))
    ...
    state_updates = {
        "wf_dsl_upgrade.targets": foreach_targets,
        "wf_dsl_upgrade.collect_targets_result": {
            "validation": validation,
            "target_count": len(targets),
            "authorized_count": len(authorized_targets),
        },
