...
- 入口 JSON 顶层字段为 `dsl_upgrade_target`；其中 `target_paths` 和 `allowed_dirs` 必填，`mode` 默认按脚本归一化为 `dry_run`，`scope_mode` 默认 `explicit`。
- facade 启动命令示例：
...
- 目标文件写入只允许发生在 `mode=apply` 且 `state.wf_dsl_upgrade.confirm_scope_result.decision=approve` 时；`.lgwf/scope_approval.json` 保存已批准的业务范围 context，不作为控制分支的 decision 来源。当前 `FOREACH` item 必须同时位于 `target_manifest.json` 与 `allowed_dirs` 内；`scope_mode=explicit` 的 `target_paths` 可传 `.lgwf` 文件或目录，目录会递归展开为 `.lgwf` 文件列表。
